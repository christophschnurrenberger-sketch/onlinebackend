...
<form method="post">
  ...
  <div class="bk-feldzeile">
    <div class="bk-feld"><label for="mail_absender">Absenderadresse</label>
      <input type="email" id="mail_absender" name="mail_absender" value="<?= Util::e($e('mail_absender')) ?>">
      <div class="bk-tipp">Sollte zur Domain des Shops gehören – sonst blocken viele Empfänger.</div></div>
    <div class="bk-feld"><label for="mail_absender_name">Absendername</label>
...
<form method="post" class="bk-block" style="margin-top:20px">
  <?= Auth::csrfFeld() ?>
  <input type="hidden" name="bereich" value="mail-test">
  <input type="hidden" name="reiter" value="mail">
  <h4 style="font-size:13px;margin:0 0 10px">Testmail senden</h4>
  <div class="bk-feldzeile" style="align-items:end">
    <div class="bk-feld" style="margin:0"><label for="test_an">An diese Adresse</label>
      <input type="email" id="test_an" name="test_an" required
             value="<?= Util::e($e('shop_email') ?: (string) $benutzer['email']) ?>"></div>
    <div><button class="bk-knopf" type="submit">Testmail senden</button></div>
  </div>
</form>
