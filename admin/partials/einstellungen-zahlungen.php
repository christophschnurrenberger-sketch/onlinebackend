  ...
  <?php foreach (Zahlung::ARTEN as $art => $vorgabeName): ?>
    ...
    <div class="bk-block">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:10px">
        <label class="bk-haken" style="margin:0;font-weight:600">
          <input type="checkbox" name="an_<?= $art ?>" value="1"
                 <?= in_array($art, $aktive, true) ? 'checked' : '' ?> <?= $bereit ? '' : 'disabled' ?>>
          <span><?= Util::e($vorgabeName) ?></span>
        </label>
        <?php if ($bereit): ?>
          <span class="bk-marke bk-marke-gruen"><i></i>Einsatzbereit</span>
        <?php else: ?>
          <span class="bk-marke bk-marke-gelb"><i></i>Zugangsdaten fehlen</span>
        <?php endif; ?>
      </div>
      ...
      <?php if ($art === 'stripe'): ?>
        ...
        <div class="bk-feld" style="margin:0">
          ...
          <div class="bk-tipp">Ohne dieses Geheimnis werden eingehende Zahlungsmeldungen abgelehnt –
            das ist Absicht.</div>
        </div>
      <?php endif; ?>
      ...
      <?php if ($art === 'paypal'): ?>
        ...
        <div class="bk-feldzeile" style="margin:0">
          <div class="bk-feld" style="margin:0">
      ...
      <?php if ($art === 'vorkasse'): ?>
        <div class="bk-feld" style="margin:12px 0 0">
          ...
          <div class="bk-tipp">Erscheint in der Bestellbestätigung und auf der Statusseite.</div>
        </div>
      <?php endif; ?>

      <?php if ($art === 'test'): ?>
        <div class="bk-hinweis bk-hinweis-warnung" style="margin:12px 0 0">
...
<p class="bk-tipp">Diese Adressen beim jeweiligen Anbieter eintragen, damit Zahlungen auch dann ankommen,
  wenn der Kunde den Browser vorzeitig schließt.</p>
<table>
  <tbody>
    <tr><td style="width:100px">Stripe</td>
        <td><div class="bk-code-kasten"><?= Util::e(Config::url('webhook.php?anbieter=stripe')) ?></div></td></tr>
    <tr><td>PayPal</td>
        <td><div class="bk-code-kasten"><?= Util::e(Config::url('webhook.php?anbieter=paypal')) ?></div></td></tr>
  </tbody>
</table>
